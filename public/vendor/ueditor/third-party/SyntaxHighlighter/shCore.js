...
if (typeof SyntaxHighlighter == "undefined")
    var SyntaxHighlighter = (function () {
        ...
        function storeHighlighter(highlighter) {
            sh.vars.highlighters[getHighlighterId(highlighter.id)] =
                highlighter;
        }
        ...
        function findElement(target, search, reverse /* optional */) {
            ...
            var nodes =
                    reverse != true ? target.childNodes : [target.parentNode],
                propertyToFind =
                    { "#": "id", ".": "className" }[search.substr(0, 1)] ||
                    "nodeName",
                expectedValue,
                found;

            expectedValue =
                propertyToFind != "nodeName"
                    ? search.substr(1)
                    : search.toUpperCase();
        ...
        function guid(prefix) {
            return (
                (prefix || "") + Math.round(Math.random() * 1000000).toString()
            );
        }
        ...
        function parseParams(str) {
            ...
            while ((match = regex.exec(str)) != null) {
                ...
                if (value != null && arrayRegex.test(value)) {
                    ...
                    value =
                        m.values.length > 0 ? m.values.split(/\s*,\s*/) : [];
                }
        ...
        function getMatches(code, regexInfo) {
            ...
            while ((match = regexInfo.regex.exec(code)) != null) {
                ...
                if (typeof resultMatch == "string")
                    resultMatch = [
                        new sh.Match(resultMatch, match.index, regexInfo.css),
                    ];
        ...
        function quickCodeHandler(e) {
            var target = e.target,
                highlighterDiv = findParentElement(
                    target,
                    ".syntaxhighlighter"
                ),
        ...
        sh.Highlighter.prototype = {
                    ...
                    for (var i = 0; i < regexList.length; i++)
                        ...
                        if (typeof regexList[i] == "object")
                            result = result.concat(
                                getMatches(code, regexList[i])
                            );

                // sort and remove nested the matches
                return this.removeNestedMatches(
                    result.sort(matchesSortCallback)
                );
            },
            ...
            removeNestedMatches: function (matches) {
                ...
                for (var i = 0; i < matches.length; i++) {
                    ...
                    for (
                        var j = i + 1;
                        j < matches.length && matches[i] !== null;
                        j++
                    ) {
                        var itemJ = matches[j];
            ...
            getLineHtml: function (lineIndex, lineNumber, code) {
                ...
                return (
                    '<div class="' + classes.join(" ") + '">' + code + "</div>"
                );
            },
